aef_style <- function() {
  ...
  ggplot2::theme(
    ...
    plot.title = ggplot2::element_text(family = font, size = 26, face = "bold", color = "#000000"),
    ...
    plot.subtitle = ggplot2::element_text(family = font, size = 18, color = "#000000", margin = ggplot2::margin(8, 0, 8, 0)),
    # This sets the font, size, type and color of text for the chart's caption
    plot.caption = ggplot2::element_text(hjust = 0, family = font, size = 14, face = "italic", color = "#5e6169"),
    # Legend format:
    # This sets the position and alignment of the legend
    legend.position = "top",
    legend.text.align = 0,
    # This sets space between legend keys when there are several rows (need guide_legend byrow to be set to TRUE)
    legend.spacing.y = ggplot2::unit(0.003, "npc"),
    ...
    strip.text = ggplot2::element_text(family = font, size = 18, color = "#000000", hjust = 0)
  )
}
